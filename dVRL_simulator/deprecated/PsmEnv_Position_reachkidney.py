"""
@brief  Script used to control the main steps of the positioning of the PAF 
        rail over the kidney target (targetk).
...
from dVRL_simulator.vrep.simObjects_reachkidney import table, rail, targetK, target #, collisionCheck 
...
class PSMEnv_Position_reachkidney(PSMEnv):
    ...
    def __init__(self, psm_num, n_substeps, block_gripper, has_object, 
            ...
            initial_pos, initial_pos_k, reward_type, 
    ...
    def _set_action(self, action):
        ...
        # TODO: computing distance
        distance_before = self._compute_distance(self.goal, pos_ee) 
        print('Distance before the step:', distance_before)
        ...
        # TODO: computing distance
        pos_ee_after, _ = self.psm.getPoseAtEE()
        distance_after = self._compute_distance(self.goal, pos_ee_after) 
        print('Distance after the step:', distance_after)
        print('Step size:', distance_before - distance_after)
    ...
    def _reset_sim(self, initial_tt_offset=0.035):
        ...
        # Setting the rail pose over the tool tip
    ...
    def _sample_goal(self):
        '''
        @Details: 
        Sets position of kidney, if not randomised the cuboid is set to 
        inital_pos_k and flat over table surface.
        If randomised:
        - x coordinate between (-2*obj_range, 2*obj_range)
        - y coordinate as x 
        - z coordinate is 0
        target_offset is add no to have the kidney inside table surface.
        Add pos_ee_projectedOnTable to lower the height.

        Orientation is sampled in randomize_k_orientation.
        
        Target is sampled randomly out of the 5 triplets of dummies on the 
        cuboid's surface. The position of the central target is set as goal.
        The rail has to lay on the kidney so that a central dummy 
        below it, called achieved_goal_central, reaches the central dummy on the 
        cuboid.
        ...
        if randomize:
            ...
            # Project EE on to the table and add the deltaGoal to that
            pos_ee_projectedOnTable, _ = self._project_point_on_table(self.initial_pos)
            kidney_pos = np.array(self.initial_pos_k) + np.array(self.target_offset) + np.array(pos_ee_projectedOnTable)
        ...
        # Once the pose is set, sample a target off the 5 sets. The goal is only 
        # the central dummy.
    ...
    def randomize_k_orientation(self):
        ...
        the difference between orientation components x, y, z can be computed in _align_to_target_orientation()
