...
from dVRL_simulator.vrep.simObjects import table, obj, target, rail
...
class PSMEnv_Position_pickplace_t_eval(PSMEnv):
    ...
    def __init__(self, psm_num, n_substeps, block_gripper, has_object,
        ...
        super(
            PSMEnv_Position_pickplace_t_eval,
            self).__init__(
            psm_num=psm_num,
    ...
    def _get_obs(self):
        ...
        if self.has_object:
            ...
            # The rail position is used as control position to computed 
            # the reward, therefore represent achieved_goal.
        ...
        # Claudia: adding check control on the grasp.
        grasp_success = self.rail.isGrasped(ignoreError=True, initialize=True)
        #print('is the rail grasped',grasp_success)
        # Claudia: printing orientation of the table.
        #pos_table, q_table = self.table.getPose(self.psm.base_handle)
        #print('the table quaternion is', q_table)
    ...
    def _define_tt_pose(self, initial_tt_offset=0.035):
        ...
        The tt is randomised inside a small volume inside target_space defined
        in the code.
    ...
    def _sample_goal(self):
        # This function defines goal and object position
        # If randomize_target = False the target is placed 1 cm above the
        # grasping site. So that the aim is just to sligtly lift the rail.
        # Otherwise the target is randomize inside the target range.
        self._simulator_step()
        ...
        if self.randomize_target:
            ...
            if self.target_in_the_air:
                z = self.np_random.uniform(0, self.target_range/2)
            else:
                z = 0

            # Add target_offset for goal.
            deltaGoal_b_homogeneous = \
                    np.append(self.np_random.uniform(-self.target_range, 
                        0, size=2), [z, 0]) \
                        + np.append(self.target_offset, [0])
            ...
            rail_pos, _ = self.rail.getPose(self.dummy_rail_handle,
                                            self.psm.base_handle)
            # Setting the target 1cm above the grasping site.
            h_offset = 0.010 
            new_pos = rail_pos
