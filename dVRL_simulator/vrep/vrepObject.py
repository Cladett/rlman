...
class vrepObject:
    ...
    # Claudia: wrapper for simxReadProximitySensor
    def readProximitySensor(self, handle, initialize = False, ignoreError = False):
        if initialize:
            vrep.simxReadProximitySensor(self.clientID, handle, vrep.simx_opmode_streaming)
                
        res, detection, detected_coordinate, detect_obj, distance_norm = vrep.simxReadProximitySensor(self.clientID, handle, vrep.simx_opmode_buffer)

        if res!=vrep.simx_return_ok and not ignoreError:
            print('Failed to detect the object')
            print(res)

        return detection, detected_coordinate, detect_obj, distance_norm
