...
from dVRL_simulator.PsmEnv_Position_rail_target_eval import PSMEnv_Position_pickplace_t_eval
import numpy as np

class PSMPickPlaceTargetEvalE4Env(PSMEnv_Position_pickplace_t_eval):
    def __init__(self, psm_num = 1, reward_type='sparse',
                 randomize_obj=True, randomize_obj_orientation=False,
                 randomize_ee=False, randomize_grasp=True,
                 randomize_target=True, action_type='continuous'):
        # This is the coordinate of the initial position of the ee on the /base
        initial_pos=np.array([0.03, 0.03, -0.07])
        # Definition of all the parameter used in the simulation 
        super(PSMPickPlaceTargetEvalE4Env, self).__init__(psm_num=psm_num, n_substeps=1,
