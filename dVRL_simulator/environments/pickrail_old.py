...
from dVRL_simulator.PsmEnv_Position_pickrail import PSMEnv_Position_pickrail
import numpy as np


class PSMPickRailEnv(PSMEnv_Position_pickrail):  #  , utils.EzPickle):
    def __init__(self, psm_num=1, reward_type='sparse',  # reward_type='sparse'
                 randomize_obj=True, randomize_obj_or=False, randomize_ee=True,
                 randomize_grasp=True, action_type='continuous'):
        initial_pos_ee = np.array([0,  0, -0.08])

        super(PSMPickRailEnv, self).__init__(
                        psm_num=psm_num,
                        n_substeps=1,
                        block_gripper=False,
                        has_object=True,
