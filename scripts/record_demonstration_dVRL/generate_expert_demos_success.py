...
@brief   This is a script is used to record expert demonstrations. 
         ONLY SUCCESSFUL EXECUTION OF TASK WILL BE SAVED. 
...
class Demo():
    ...
    def __init__(self, n_episodes, dic_path, task):
        ...
        # Initialise data structures to save success values
    ...
    def run_episode(self, count_success):
        ...
        @brief  Each episode runs a reset of the environment and whole 
                demonstration 
        """
        # Defining variable for the demos to store
        actions = []
        observ = []
        rewards = []
        episode_starts = []
        ...
        if self.task == 'pickrail':
            prev_obs, actions, observ, rewards, episode_starts = self.approach(
                            prev_obs, actions, observ, rewards, episode_starts )
            prev_obs, actions, observ, rewards, episode_starts,info_grasp = self.grasp(
                            prev_obs, actions, observ, rewards, episode_starts)
        else: 
            prev_obs, actions, observ, rewards, episode_starts = self.approach(
                            prev_obs, actions, observ, rewards, episode_starts )
            prev_obs, actions, observ, rewards, episode_starts = self.grasp( 
                            prev_obs, actions, observ, rewards, episode_starts )
            prev_obs, actions, observ, rewards, episode_starts, info = self.reach(
                            prev_obs, actions, observ, rewards, episode_starts )
        
        # Saving the demonstration only if successful 
        if self.task == 'pickrail':
            if info_grasp['is_success']:
                count_success +=1
                self.actions.extend(actions) 
                self.obs.extend(observ)
    ...
    def approach(self, prev_obs, actions, observ, rewards, episode_starts ):
        """
        ...
        for i in range(self.APPROACH_STEPS):
            ...
            observ.append(np.append(prev_obs[0:7],prev_obs[10:13]))
            rewards.append(reward) 
            episode_starts.append(i == 0) 
            prev_obs = obs
        return prev_obs, actions, observ, rewards, episode_starts

    def grasp(self, prev_obs, actions, observ, rewards, episode_starts):
        """
    ...
    def reach(self, prev_obs, actions, observ, rewards, episode_starts):
        """
        ...
        return prev_obs, actions, observ, rewards, episode_starts, info
